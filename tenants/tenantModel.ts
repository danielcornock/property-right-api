...
tenantSchema.pre('save', async function setPropertyName(
  this: ITenantDocument,
  next: INext
) {
  if (this.propertyId) {
    const { name }: IPropertyDocument = await databaseService.findById(
      Property,
      this.propertyId
    );
...
tenantSchema.pre('save', async function setAvatarDetails(
  this: ITenantDocument,
  next: INext
) {
...
tenantSchema
  ...
  .get(function getInitials(this: ITenantDocument) {
